  ...
  methods: {
    ...
    tabClick(index) {
      ...
      if (
        ...
        if (this.saveYItem[this.currentType] == 0) {
          this.$refs.scroll.scrollTo(0, -this.tabOffsetTop, 0);
        } else {
          setTimeout(() => {
            this.$refs.scroll.scrollTo(0, this.saveYItem[this.currentType], 0);
          }, 300);
        }
      }
    },
    ...
    contentScroll(position) {
      ...
      this.saveYItem[this.currentType] = position.y;
      // console.log(this.saveYItem[this.currentType]);
    },

    //上拉加载更多
    loadMore() {
      this.getHomeGoods(this.currentType);
      console.log("加载更多"); //getHomeGoods中已经在后面使用这个函数
    },

    swiperImageLoad() {
      //获取轮播图中图片的高度
      //所有组件都有这个$el属性，获取组件中的元素,组件根元素
